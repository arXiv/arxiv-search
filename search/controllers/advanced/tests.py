...
class TestAdvancedSearchForm(TestCase):
    ...
    def test_end_date_bounding(self):
        ...
        self.assertTrue(form.validate())
        self.assertEqual(
            form.date.to_date.data, date(year=2012, month=12, day=31)
        )
        ...
        self.assertTrue(form.validate())
        self.assertEqual(
            form.date.to_date.data, date(year=2012, month=2, day=29)
        )
        ...
        self.assertTrue(form.validate())
        self.assertEqual(
            form.date.to_date.data, date(year=2016, month=6, day=30)
        )
    ...
    def test_input_whitespace_is_stripped(self):
        ...
        self.assertEqual(
            form.terms[0].term.data, "foo", "Whitespace should be stripped."
        )
...
class TestUpdatequeryWithClassification(TestCase):
    ...
    def test_physics_is_selected_specific_archive(self):
        ...
        self.assertEqual(
            q.classification,
            [{"archive": {"id": "hep-ex"}, "group": {"id": "grp_physics"}}],
        )
    ...
    def test_physics_is_selected_specific_archive_plus_other_groups(self):
        ...
        self.assertEqual(
            ...
            [
                {"archive": {"id": "math"}},
                {"group": {"id": "grp_physics"}, "archive": {"id": "hep-ex"}},
            ],
        )
...
class TestUpdateQueryWithDates(TestCase):
    ...
    def test_specific_year_is_selected(self):
        ...
        self.assertIsInstance(q, AdvancedQuery)
        self.assertEqual(
            q.date_range.end_date.date(), date(year=2000, month=1, day=1)
        )
        self.assertEqual(
